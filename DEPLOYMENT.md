...
A          karonlinelive.com        185.199.108.153               DNS only
A          karonlinelive.com        185.199.109.153               DNS only
A          karonlinelive.com        185.199.110.153               DNS only
A          karonlinelive.com        185.199.111.153               DNS only
CNAME      www                      KARONLINE64.github.io         DNS only
```

Ces IP sont les serveurs GitHub Pages (fixes). Ne rattachez jamais `karonlinelive.com` ni `www.karonlinelive.com` a un Cloudflare Tunnel : le tunnel doit rester reserve a `api.karonlinelive.com`.

**Chez votre registrar**:
- Connexion → DNS
- Modifier les enregistrements A
- Pointer vers les IP ci-dessus
- Dans Cloudflare, laisser `karonlinelive.com` et `www` en **DNS only** (nuage gris)
...
4. **Téléchargement (statique)**:
   ```
   Bouton "Télécharger KaronlineBox" → GET /downloads/KaronlineBox_V90_Setup.exe
   → Télécharge l'installateur depuis GitHub Pages, sans tunnel API
   ```
...
**Cause**: Fichier d'installation absent du dossier `downloads/` publie avec GitHub Pages.
**Solution**: Verifier que `downloads/KaronlineBox_V90_Setup.exe` est present avant publication.
